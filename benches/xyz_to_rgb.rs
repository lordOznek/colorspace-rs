...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let xyzs = colorchecker::XYZ_D65
        ...
        .map(|(_, x)| XYZf32::from(*x))
        .collect::<Vec<_>>();
    c.bench_function("xyz_to_rgb_slice_f32", move |b| {
        b.iter(|| black_box(xyz_slice_to_rgb(&mtx32, &xyzs)))
    });
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "avx"))]
    {
        ...
        c.bench_function("xyz_to_rgb_slice_avx_planes", move |b| {
            b.iter(|| black_box(xyz_slice_to_rgb_avx_planes::<simdeez::avx2::Avx2>(&mtx32, &xs, &ys, &zs)))
        });
    }
}
