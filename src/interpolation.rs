...
pub trait SpragueCoefficients {
    ...
    fn coeff_a(r: &[Self::Item], i: usize) -> [Self::Item; 6];
}
...
impl SpragueCoefficients for f32 {
    ...
    fn coeff_a(r: &[f32], i: usize) -> [f32; 6] {
        let a0p = r[i];
        let a1p = (2.0 * r[i - 2] - 16.0 * r[i - 1] + 16.0 * r[i + 1] - 2.0 * r[i + 2]) / 24.0;
        let a2p = (-r[i - 2] + 16.0 * r[i - 1] - 30.0 * r[i] + 16.0 * r[i + 1] - r[i + 2]) / 24.0;
        let a3p = (-9.0 * r[i - 2] + 39.0 * r[i - 1] - 70.0 * r[i] + 66.0 * r[i + 1] - 33.0 * r[i + 2] + 7.0 * r[i + 3]) / 24.0;
        let a4p = (13.0 * r[i - 2] - 64.0 * r[i - 1] + 126.0 * r[i] - 124.0 * r[i + 1] + 61.0 * r[i + 2] - 12.0 * r[i + 3]) / 24.0;
        let a5p = (-5.0 * r[i - 2] + 25.0 * r[i - 1] - 50.0 * r[i] + 50.0 * r[i + 1] - 25.0 * r[i + 2] + 5.0 * r[i + 3]) / 24.0;
...
impl SpragueCoefficients for f64 {
    ...
    fn coeff_a(r: &[f64], i: usize) -> [f64; 6] {
        let a0p = r[i];
        let a1p = (2.0 * r[i - 2] - 16.0 * r[i - 1] + 16.0 * r[i + 1] - 2.0 * r[i + 2]) / 24.0;
        let a2p = (-r[i - 2] + 16.0 * r[i - 1] - 30.0 * r[i] + 16.0 * r[i + 1] - r[i + 2]) / 24.0;
        let a3p = (-9.0 * r[i - 2] + 39.0 * r[i - 1] - 70.0 * r[i] + 66.0 * r[i + 1] - 33.0 * r[i + 2] + 7.0 * r[i + 3]) / 24.0;
    ...
    T: SpdElement + SpragueCoefficients<Item = T>,
{
    pub fn new(vspd: &VSPD) -> InterpolatorSprague<f64> {
        ...
        let y1 = f64::coeff_c0().iter().zip(vspd.iter()).map(|(c, s)| *c * s.v).sum::<f64>();

        let y2 = f64::coeff_c1().iter().zip(vspd.iter()).map(|(c, s)| *c * s.v).sum::<f64>();
    ...
    pub fn evaluate(&self, x: T) -> T {
        let i = (self.x.iter().position(|t| x < *t).unwrap() - 1).max(2).min(self.x.len() - 4);
        let dx = (x - self.x[i]) / (self.x[i + 1] - self.x[i]);

        let a = T::coeff_a(&self.y, i);

        a[0] + a[1] * dx + a[2] * dx.powi(2) + a[3] * dx.powi(3) + a[4] * dx.powi(4) + a[5] * dx.powi(5)
    }
}
